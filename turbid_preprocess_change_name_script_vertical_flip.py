...
if TURBID:
    transform_v2 = A.Compose([
        ...
        A.OneOf([
            A.CoarseDropout(num_holes_range=(1, 100), hole_height_range=(0.001, 0.015), hole_width_range=(0.001, 0.015),
                            fill=(255, 255, 255), p=0.95),
            A.CoarseDropout(num_holes_range=(1, 100), hole_height_range=(0.001, 0.015), hole_width_range=(0.001, 0.015),
                            fill=(180, 180, 180), p=0.95),
            A.CoarseDropout(num_holes_range=(1, 100), hole_height_range=(0.001, 0.015), hole_width_range=(0.001, 0.015),
                            fill=(100, 90, 70), p=0.95),
        ], p=1.0),
    ])
else:
    transform_v2 =A.Compose([A.VerticalFlip(p=1.0)])
...
for subdir in subdirs:
    ...
    for filename in os.listdir(in_images_dir):
        ...
        # 2. חיפוש קובץ ה-TXT בתיקיית labels, היפוך ושמירה
        ...
        if os.path.exists(txt_file_path):
            ...
            for line in lines:
                ...
                new_coords = []
                # ב-YOLO הקואורדינטות הולכות בזוגות: x1 y1 x2 y2...
                for i in range(len(coords)):
                    if i % 2 == 0:
                        # X נשאר זהה (אין היפוך אופקי)
                        new_coords.append(coords[i])
                    else:
                        # Y מתהפך
                        flipped_y = 1.0 - coords[i]
                        # מוודא שלא חרגנו מהגבולות
                        flipped_y = max(0.0, min(flipped_y, 1.0))
                        new_coords.append(flipped_y)
...
print("\n==========================================")
print(f"Done! Processed {images_processed} images and mathematically flipped {labels_updated} YOLO label files.")