            ...
            for line in lines:
                ...
                for i in range(len(coords)):
                    if i % 2 == 0:
                        # === הלב של התיקון ===
                        # X מתהפך במקום Y
                        flipped_x = 1.0 - coords[i]
                        # מוודא שלא חרגנו מהגבולות
                        flipped_x = max(0.0, min(flipped_x, 1.0))
                        new_coords.append(flipped_x)
                    else:
                        # Y נשאר זהה
...
print("\n==========================================")
print(f"Done! Processed {images_processed} images and mathematically flipped {labels_updated} YOLO label files horizontally.")